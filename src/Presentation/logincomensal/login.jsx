...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch("https://restaurantes20194359.azurewebsites.net/backend/Cliente", {
        method: 'POST',
        headers: {
  ...
  <form>
    <div className="field">
      ...
      <input autoComplete="off"value={codigo}
        onChange={(e) => setEmail(e.target.value)} id="logemail" placeholder="Código" className="input-field" name="logemail" type="email" />
    </div>
    <div className="field">
      <svg className="input-icon" viewBox="0 0 500 500" xmlns="http://www.w3.org/2000/svg">
      ...
      <input autoComplete="off" value={password}
        onChange={(e) => setPassword(e.target.value)} id="logpass" placeholder="contraseña" className="input-field" name="logpass" type="password" />
    </div>
    <div className='centrar'><button onClick={handleSubmit} id='btn' type="submit">Login</button></div>
